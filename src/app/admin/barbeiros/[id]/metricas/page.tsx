...
import { ArrowLeft, BarChart3, Calendar, DollarSign, Scissors, Trophy, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function BarberMetricsPage() {
  ...
  const loadMetrics = async (selectedPeriod: 'day' | 'week' | 'month' | 'all' = period) => {
    ...
    setErrorMsg(null);
    try {
      const res = await getBarberMetricsAction(barberId, selectedPeriod);
    ...
    <div className="min-h-screen bg-black text-slate-100 flex flex-col selection:bg-amber-500/30">
      ...
      <header className="sticky top-0 z-50 border-b border-zinc-900 bg-black/80 backdrop-blur-md">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold bg-zinc-900 hover:bg-zinc-800 text-slate-200 border border-zinc-800 transition-colors"
          >
      ...
      <main className="flex-grow container mx-auto px-4 py-8 space-y-8 max-w-5xl">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm">Acompanhe evolução temporal dos atendimentos e serviços preferidos.</p>
          </div>
